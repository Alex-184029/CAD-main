...
def get_item_list():
    ...
    try:
        if task_type == 'Door':
            ...
            logfile = os.path.join(dwg_public, (dwgname + '_ParallelWindow.json'))
            res = read_json_to_dict(logfile)
        else:
...
def recongDoor2(task_id):
    ...
    with app.app_context():
        ...
        # 提交请求进行解析
        try:
            res = post_url(url, os.path.join(dwg_public, dwgname))
            if res is None or res['status'] != 'success':
                print('post_url res:', res)
                return None
...
def recongWindow2(task_id):
    print('Here is recongWindow2.')
    url = 'http://192.168.131.128:5005/parse_window'  # 虚拟机
    with app.app_context():
        target_task = Task.query.filter_by(task_id=task_id).first()
        dwgname = target_task.drawing_name
        print('图纸 %s 开始统计' % dwgname)
        ...
        try:
            ...
            res_parse = res['res']
            tmpname = os.path.splitext(dwgname)[0]
            jsonpath = os.path.join(dwg_public, tmpname + '_ParallelWindow.json')
            print('jsonpath:', jsonpath)
            write_dict_to_json(res_parse, jsonpath)
            imgpath = os.path.join(img_public, tmpname + '_Origin_Window.jpg')
            writeBase64(res['img'], imgpath)
            target_task.status = 'sucess'
        except Exception as e:
            print('error in recongWindow2:', e)
            target_task.status = 'fail'
