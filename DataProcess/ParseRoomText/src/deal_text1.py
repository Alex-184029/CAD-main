...
def createDataset():
    labelpath = r'E:\School\Grad1\CAD\MyCAD2\CAD-main\DataProcess\ParseRoomText\data\labels'
    outpath = r'E:\School\Grad1\CAD\DeepLearn\TextClassify\Chinese-Text-Classification-Pytorch\RoomLabels\data'
    os.makedirs(outpath, exist_ok=True)
    rooms = room_type.keys()
    print('rooms:', rooms)
    dataset_list = []
    for i, room in enumerate(rooms):
        with open(os.path.join(labelpath, room + '.txt'), 'r', encoding='utf-8') as f:
            labels = f.readlines()
        for label in labels:
    ...
    dataset_list += dataset_list    # 复制两份扩充数据集
...
if __name__ == '__main__':
    # txtpath = '../data/(1#2#3#标准层公区报建)电梯厅平面图 rev-05-1.txt'
    # txtpath = '../data/(T3) 12#楼105户型平面图（镜像）-1.txt'
    # readTxt(txtpath)
    # readTxtBatch()
    # test1()
    # doSelect2()
